...
**Descripción Técnica Detallada:**
El diagrama (confirmat en la font) representa la interacció de 5 missatges del protocol Needham-Schroeder per a la distribució de claus secretes i l'autenticació mútua. Els elements clau inclouen Alice (A) i Bob (B), el Servidor de Distribució de Claus (KDC), les claus secretes $K_{A,KDC}$ i $K_{B,KDC}$, la clau de sessió temporal $K_{A,B}$, i els *nonces* $R_{A1}, R_{A2}, R_B$ utilitzats per assegurar la frescor del missatge. El **Ticket** és un missatge xifrat per $K_{B,KDC}$ que conté A i $K_{A,B}$. Alice rep el Ticket en el missatge 2 i el reenvia a Bob (missatge 3), ja que només Bob el pot desxifrar, provant així a B l'origen de la clau de sessió des del KDC.
...
**Descripción Técnica Detallada:**
Aquesta figura (implícita en la font, ja que es refereix a un diagrama que cal etiquetar) representa el diagrama d'esdeveniments per a l'Escenari 1 utilitzat per al càlcul dels rellotges lògics. El diagrama il·lustra la causalitat entre els processos $P_1$ a $P_4$ mitjançant esdeveniments locals (punts) i missatges (fletxes). L'etiquetatge amb rellotges de Lamport d'aquest diagrama condueix als següents resultats finals:
...
**Descripción Técnica Detallada:**
Aquesta figura (implícita en la font, ja que es refereix a un diagrama que cal etiquetar) representa el diagrama d'esdeveniments per a l'Escenari 2. Mostra una configuració de causalitat diferent a l'Escenari 1 entre els processos $P_1$ a $P_4$. Aquest diagrama és la base visual per determinar els segells de temps de Lamport i vectorials per al segon conjunt de resultats.
...
##### Needham-Schroeder Public key protocol (NSPK) (https://www.f4tm4n.com/needham-
schroeder-public-key-protocol-nspk)
