...
**Descripción Técnica Detallada:**
Aquesta figura (implícita en la font, ja que es refereix a un diagrama que cal etiquetar) representa el diagrama d'esdeveniments per a l'Escenari 1, utilitzat per al càlcul dels rellotges lògics (Lamport i vectorials). El diagrama il·lustra la causalitat entre els processos $P_1$ a $P_4$ mitjançant esdeveniments locals (punts) i missatges (fletxes). L'etiquetatge amb rellotges de Lamport d'aquest diagrama condueix als següents resultats finals:
...
**Descripción Técnica Detallada:**
Aquesta figura (implícita en la font, ja que es refereix a un diagrama que cal etiquetar) representa el diagrama d'esdeveniments per a l'Escenari 2. Mostra una configuració de causalitat diferent a l'Escenari 1 entre els processos $P_1$ a $P_4$. Aquest diagrama és la base visual per determinar els segells de temps de Lamport i vectorials per al segon conjunt de resultats, que són els següents per a Lamport:
...
##### Graf exercici 3.A
*(Nota: El codi Mermaid ja s'ha integrat al cos principal del document, sota la Figura Image001, complint amb l'objectiu de retenir el codi gràfic)*.