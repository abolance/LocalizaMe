...
La imatge és un diagrama de flux *mermaid* que detalla el flux d'interacció de l'Authorization Code Grant Type d'OAuth 2.0 en 8 passos. Els rols participants són el Resource Owner (Usuari), el Client (Aplicació), l'Authorization Server (AS) i el Resource Server (RS). El flux comença quan l'Usuari inicia l'acció (1) i és redirigit a l'AS (2) per autenticar-se i donar consentiment (3). L'AS envia un Codi d'Autorització (4). El Client utilitza un **Canal Segur (Back-Channel)** per intercanviar aquest Codi (5) per un Access Token amb l'AS (6). Finalment, el Client utilitza l'Access Token per sol·licitar el recurs protegit del RS (7), que valida el Token i retorna les dades (8).
...
**Descripción Técnica Detallada:**
El diagrama representa conceptualment la interacció de 5 missatges del protocol Needham-Schroeder per a la distribució de claus secretes. Els elements identificats inclouen Alice (A) i Bob (B), el Servidor d'Autenticació KDC, les claus secretes $K_{A,KDC}$ i $K_{B,KDC}$, la clau de sessió temporal $K_{A,B}$, i els *nonces* $R_{A1}, R_{A2}, R_B$ utilitzats per evitar atacs de repetició. El focus recau en el **Ticket**, un missatge xifrat per $K_{B,KDC}$ que conté A i $K_{A,B}$. Alice rep el Ticket en el missatge 2, però només el pot reexpedir a Bob (missatge 3), assegurant a Bob l'origen de la clau de sessió per part del KDC.
...
| **4** | Bob desxifra el ticket, identifica a Alice i obté la clau $K_{A,B}$. | Bob respon a Alice amb $R_{A2}-1$ per informar a Alice de que realment es Bob. I Bob envia un nonce $R_B$ per verifivar que Alice està activa. |
| **5** | Alice desxifra el missatge de Bob i modifica $R_B$ i el retorna xifrat amb la clau de sessió com $R_B-1$. | Al completar la seqüència Alice i Bob disposen de la clau de sessió $K_{A,B}$ i asseguren la comunicació xifrada. |
...
**Descripción Técnica Detallada:**
Aquesta figura representa el diagrama d'esdeveniments per a l'Escenari 1 utilitzat per al càlcul dels rellotges lògics. El diagrama, que mostra la causalitat entre els processos ($P_1, P_2, P_3, P_4$) a través de fletxes de missatges i esdeveniments locals (punts), defineix la seqüència d'interaccions. Aquest diagrama és la base visual per determinar els segells de temps de Lamport, que resulten en els valors finals següents per a cada procés:
...
**Descripción Técnica Detallada:**
Aquesta figura representa el diagrama d'esdeveniments per a l'Escenari 2. És un diagrama de causalitat diferent de l'Escenari 1, utilitzat també per calcular els valors dels rellotges lògics (Lamport i vectorials) en una configuració alternativa de missatges entre els processos $P_1, P_2, P_3, P_4$. Aquest diagrama condueix als següents valors finals per a Lamport:
...
##### Els rellotges vectorials s’aplicuen per detectar conflictes de versions d’una mateixa dada que ha estat actualitzada de manera concurrent en diferents servidors (rèpliques).
